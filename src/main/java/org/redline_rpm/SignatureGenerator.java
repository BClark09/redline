...
public class SignatureGenerator {
	...
	protected static final int DEFAULT_SIGNATURE_SIZE = 287;
	protected final int privateKeySignatureSize;
	...
	public SignatureGenerator( PGPPrivateKey privateKey ) {
		this(privateKey, DEFAULT_SIGNATURE_SIZE);
	}

	public SignatureGenerator( PGPPrivateKey privateKey, int privateKeySignatureSize ) {
		this.privateKey = privateKey;
		this.enabled = privateKey != null;
		this.privateKeySignatureSize = privateKeySignatureSize > 0 ? privateKeySignatureSize : DEFAULT_SIGNATURE_SIZE;
	}

	public SignatureGenerator( File privateKeyRingFile, String privateKeyId, String privateKeyPassphrase) {
		this(privateKeyRingFile, privateKeyId, privateKeyPassphrase, DEFAULT_SIGNATURE_SIZE);
	}

	public SignatureGenerator( File privateKeyRingFile, String privateKeyId, String privateKeyPassphrase, int privateKeySignatureSize ) {
		this.privateKeySignatureSize = privateKeySignatureSize > 0 ? privateKeySignatureSize : DEFAULT_SIGNATURE_SIZE;
	...
	public void prepare( Signature signature ) {
		if ( enabled ) {
			headerOnlyRSAEntry = ( Entry< byte[]> ) signature.addEntry( RSAHEADER, privateKeySignatureSize );
			headerAndPayloadPGPEntry = ( Entry< byte[]> ) signature.addEntry( LEGACY_PGP, privateKeySignatureSize );
		}
	}
